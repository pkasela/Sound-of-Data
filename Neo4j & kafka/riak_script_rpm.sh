...
finalize_yum_repo ()
{
  ...
  echo "Generating yum cache for basho_riak-source..."
  yum -q makecache -y --disablerepo='*' --enablerepo='basho_riak-source'
}

finalize_zypper_repo ()
{
  zypper --gpg-auto-import-keys refresh basho_riak
  zypper --gpg-auto-import-keys refresh basho_riak-source
}
...
main ()
{
  ...
  if [ "${os}" = "sles" ] || [ "${os}" = "opensuse" ]; then
    yum_repo_path=/etc/zypp/repos.d/basho_riak.repo
  else
...
#followed by a 'sudo yum install riak' should do the trick
